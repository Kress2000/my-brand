...
const filterBLOGS = async () => {
  ...
  let codeBlogs = [];
  if (allBlogsFromDb) {
    codeBlogs = allBlogsFromDb.filter((blog) =>
      blog.category === "Code" ? blog : null
    );
  }
  console.log("Code blogs got: ", codeBlogs);
};
filterBLOGS();
        ...
        <div class="box" onclick="clickedBox(${blog.id})">
            ...
            <div class="userDecision">
                <div class="subBox">
                    ...
                    <div class="number heartNumber" >${
                      blog.userActions.likes
                    }</div>
                </div>
                <div class="subBox">
                    <div class="views">
                    ...
                    <div class="number eyeNumber" >${
                      blog.userActions.views
                    }</div>
                </div>
                <div class="subBox">
                    <div class="comments">
                    ...
                    <div class="number commentNumber">${
                      blog.userActions.comments.length
                    }</div>
                </div>
            </div>
        </div>
...
function clickedBox(id) {
  ...
  if (blog) {
    let blogData = blog[0];
    const blogfromAllArr = blogDataAdd.filter((blogfromAll) =>
      blogfromAll.id === blogData.id ? blogfromAll : null
    );
    ...
    newcommentsArr.forEach((comment, commentId) => {
      ...
      userCommentsposted.innerHTML += `
            <div class="commentline" id="${wholecommentId}">
                ...
                <div>
                    <i class="fa-solid fa-trash deleteComent" onclick='deleteComent(${JSON.stringify(
                      data
                    )})' id="${commentId}"></i>
                </div>
            </div>
